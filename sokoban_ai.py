			...
			if board.is_legal_move('l'):
				...
				boxLoc in boxLocs:
					for storLoc in storLocs:
						if  boxLoc(1) == (player_pos(1) - 1) && grid[player_pos(1) - 2][boxLoc(2)] != '#':
				...
				boxLoc in boxLocs:
					for storLoc in storLocs:
						if  boxLoc(2) == (player_pos(2) - 1) && grid[player_pos(1)][boxLoc(2)-1] != '#':
...
class Game:
	...
	def corner_deadlock(self):
		...
		for coord in boxCoords:
			i = coord(1)
			j = coord(2)
			if (board_grid[i-1][j] == '#' && board_grid[i][j-1] == '#') || (board_grid[i+1][j] == '#' && board_grid[i][j-1] == '#') || (board_grid[i][j+1] == '#' && board_grid[i-1][j] == '#') ||  (board_grid[i][j+1] == '#' && board_grid[i+1][j] == '#')):
				return True
		...
		for coord in boxCoords:
			...
			if j == 1 || j == (h-2):
				for store in storLoc:
					if store(2) == 1 || store(2) == (v-2):
						if store(1) >= 1 || store(1) <= (h-2):
							return False
			elif i == 1 || i == (v-2):
				for store in storLoc:
					if store(1) == 1 || store(1) == (h-2):
	...
	def square_block_checker(self, i, j, board_grid):
		...
		elif board_grid[i-1][j] == '$' && board_grid[i][j-1] == '$':
			return True
		return False


	def isDeadEnd(self):
		if self.simple_deadlock() || self.pre_corner_deadlock() || self.square_block_deadend() :
			return True
		return False
